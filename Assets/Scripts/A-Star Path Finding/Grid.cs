...
public class Grid : MonoBehaviour
{
    ...
    public LayerMask PlayerMask;//This is the mask that the program will look for when trying to find obstructions to the path.
    ...
    void CreateGrid()
    {
        ...
        for (int x = 0; x < iGridSizeX; x++)//Loop through the array of nodes.
        {
            for (int y = 0; y < iGridSizeY; y++)//Loop through the array of nodes
            {
                ...
                bool bPlayer = false;//Make the node a wall
                ...
                if (Physics.CheckSphere(worldPoint, fNodeRadius, WallMask))
                {
                    ...
                    //Instantiate(playerIndicator, worldPoint, Quaternion.identity);
                    //Debug.Log("Wall Found: " + worldPoint);
                }
                //if (Physics.CheckSphere(worldPoint, fNodeRadius * 2, PlayerMask))
                //{
                //    bPlayer = true;//Object is not a wall
                //    //Instantiate(playerIndicator, worldPoint, Quaternion.identity);
                //}

                NodeArray[x, y] = new Node(bPlayer, Wall, worldPoint, x, y);//Create a new node in the array.
            }
        }
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        if (NodeArray != null)//If the grid is not empty
        {
            foreach (Node n in NodeArray)//Loop through every node in the grid
            {
                ...
                //{
                //    Gizmos.color = gridFloorColor;//Set the color of the node
                //}
                if (n.bIsPlayer)//If the current node is a wall node
                {
               ...
               //}
                ...
                Gizmos.DrawCube(n.vPosition, new Vector3(fNodeDiameter - fDistanceBetweenNodes, .1f, fNodeDiameter - fDistanceBetweenNodes));//Draw the node at the position of the node.
            }
        }
    }
}
