...
public class PathFinding : MonoBehaviour
{
    ...
    public List<Node> FindPath(Vector3 a_StartPos, Vector3 a_TargetPos)
    {
        ...
        while (OpenList.Count > 0)//Whilst there is something in the open list
        {
            ...
            {
                FinalPathForPlayer = GetFinalPath(StartNode, TargetNode);//Calculate the final path
            }
            ...
            {
                if (!NeighborNode.bIsWall || NeighborNode.bIsPlayer || ClosedList.Contains(NeighborNode))//If the neighbor is a wall or has already been checked
                {
                    continue;//Skip it
                }
        ...
        return FinalPathForPlayer;
    }



    public List<Node> GetFinalPath(Node a_StartingNode, Node a_EndNode)
    {
